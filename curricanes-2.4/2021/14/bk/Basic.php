...
namespace Wagento\FixMercadoPago\Model\Preference;
...
class Basic extends \MercadoPago\Core\Model\Preference\Basic
{
    ...
    protected function getItems($order, $config)
    {
        ...
        $total_item  += Round::roundWithSiteId($order->getShippingAmount(), $this->getSiteId());
        $order_amount = Round::roundWithSiteId($order->getGrandTotal(), $this->getSiteId());

        if (!$order_amount) {
            $order_amount = Round::roundWithSiteId(
                $order->getPrice() + $order->getShippingAmount(),
                $this->getSiteId()
            );
        }
    ...
    protected function calculateBaseTaxAmount(&$arr, $order, $config)
    {
        if ($order->getTaxAmount() > 0) {
            $arr[] = [
                ...
                "category_id" => $config['category_id'],
                "quantity"    => 1,
                "unit_price"  => Round::roundWithSiteId($order->getTaxAmount(), $this->getSiteId())
            ];
        }
        ...
        try {
            ...
            if ($order->canShip()) {
                ...
                $arr['items'][] = [
                    ...
                    "category_id" => $config['category_id'],
                    "quantity"    => 1,
                    "unit_price"  => Round::roundWithSiteId($order->getShippingAmount(), $this->getSiteId())
                ];
            }
